...
'''
def score2percentile(score, ref):
    percentile = percentileofscore(np.array(ref).ravel(), score)
            ...
            if attn_save_path is not None:
                ...
                if ref_scores is not None:
                    ...
                    for score_idx in range(len(A)):
                        ...
                        print("score_idx shape:", np.shape(score_idx))
                        print("A[score_idx] shape:", np.shape(A[score_idx]))
                        print("score2percentile output shape:", np.shape(score2percentile(A[score_idx], ref_scores)))

                        A[score_idx] = score2percentile(A[score_idx], ref_scores)
                        #A[score_idx] = score2percentile(float(A[score_idx]), ref_scores)
                    ''' 
                    for score_idx in range(len(A)):
                        score_val = A[score_idx].item()  # safely extracts scalar from array
                        A[score_idx] = score2percentile(score_val, ref_scores)
